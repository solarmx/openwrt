...
OUT=$(OPENWRT_TAG=test REPO_ROOT="$T" "$SCRIPT" 2>/dev/null || true)
TEXT=$(printf '%s' "$OUT" | jq -r '.notices[]? | select(.name=="foo") | .text' 2>/dev/null || true)
LICVAL=$(printf '%s' "$OUT" | jq -r '.notices[]? | select(.name=="foo") | .license' 2>/dev/null || true)
...
printf 'bar - 2.0\n' > "$T/bin/targets/x/y/rootfs.manifest"
OUT=$(OPENWRT_TAG=test REPO_ROOT="$T" "$SCRIPT" 2>/dev/null || true)
TEXT=$(printf '%s' "$OUT" | jq -r '.notices[]? | select(.name=="bar") | .text' 2>/dev/null || true)
...
TEXT=$(printf '%s' "$OUT" | jq -r '.notices[]? | select(.name=="qux") | .text' 2>/dev/null || true)
...
LICVAL=$(printf '%s' "$OUT" | jq -r '.notices[]? | select(.name=="mystery") | .license' 2>/dev/null || true)
TEXT=$(printf '%s' "$OUT" | jq -r '.notices[]? | select(.name=="mystery") | .text' 2>/dev/null || true)
